...
int MALLOCTRACE_ERR_CODE = ERR_UNINITIALIZED;
...
static void malloctrace_init() {
  if (MALLOCTRACE_ERR_CODE == ERR_OK) return;
...
void *malloc(size_t size) {
  if (!_original_malloc) malloctrace_patch_functions();
  if (MALLOCTRACE_ERR_CODE != ERR_OK) malloctrace_init();
...
void free(void *ptr) {
  if (!_original_free) malloctrace_patch_functions();
  if (MALLOCTRACE_ERR_CODE != ERR_OK) malloctrace_init();
  _original_free(ptr);
  handle_free(ptr);
}
...
void *calloc(size_t nmemb, size_t size) {
  if (!_original_calloc) malloctrace_patch_functions();
  if (MALLOCTRACE_ERR_CODE != ERR_OK) malloctrace_init();
...
void *realloc(void *ptr, size_t size) {
  if (!_original_realloc) malloctrace_patch_functions();
  if (MALLOCTRACE_ERR_CODE != ERR_OK) malloctrace_init();
...
ALWAYS_INLINE static void handle_malloc(void *ptr, size_t size) {
  if (!MALLOCTRACE_ACTIVE || !MALLOC_HOOK_ACTIVE ||
      MALLOCTRACE_DEPTH > MAX_RECURSION_DEPTH) {
    return;
  }
  if (ptr == NULL || size == 0x0) return;
  ENTER_HANDLER_SECTION(MALLOC);
  AllocationDesc allocation = {.chunk = {.address = ptr, .size = size},
...
ALWAYS_INLINE static void handle_free(void *ptr) {
  if (!MALLOCTRACE_ACTIVE || !FREE_HOOK_ACTIVE ||
      MALLOCTRACE_DEPTH > MAX_RECURSION_DEPTH) {
    return;
  }
  if (ptr == NULL) return;
...
ALWAYS_INLINE static void handle_calloc(void *ptr, size_t nmemb, size_t size) {
  if (!MALLOCTRACE_ACTIVE || !CALLOC_HOOK_ACTIVE ||
      MALLOCTRACE_DEPTH > MAX_RECURSION_DEPTH) {
    return;
  }
  if (ptr == NULL || nmemb * size == 0x0) return;
  ENTER_HANDLER_SECTION(CALLOC);
  AllocationDesc allocation = {.chunk = {.address = ptr, .size = nmemb * size},
                               .backtrace = get_backtrace()};
  if (heap_map_insert(MALLOCTRACE_HEAP_MAP, &allocation) == -1) {
    malloctrace_error("Couldn't insert into heap map: OutOfMemory!\n");
  }
...
ALWAYS_INLINE static void handle_realloc(void *new_ptr, void *ptr,
                                         size_t size) {
  if (!MALLOCTRACE_ACTIVE || !REALLOC_HOOK_ACTIVE ||
      MALLOCTRACE_DEPTH > MAX_RECURSION_DEPTH) {
    return;
  }
  if (ptr == NULL) {
  ...
  if (new_ptr == ptr) {
    ...
    handle_free(ptr);
    AllocationDesc allocation = {.chunk = {.address = new_ptr, .size = size},
                                 .backtrace = get_backtrace()};
